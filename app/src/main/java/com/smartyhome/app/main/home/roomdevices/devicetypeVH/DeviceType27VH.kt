...
import kotlinx.android.synthetic.main.device_item_list_type_27.view.*

class DeviceType27VH(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(
        ...
        if (deviceModelVH.d1?.br != -0.0) {
            var progess = ((deviceModelVH.d1?.br)?.times(100))?.toInt()
            seekbarBRd1.setProgress(progess ?: 0)
            tvBrightnessd1.setText(
                tvBrightnessd1.context.getString(R.string.brightness).plus(" $progess %")
            )
        }
        if (deviceModelVH.d2?.br != -0.0) {
            var progess = ((deviceModelVH.d2?.br)?.times(100))?.toInt()
        ...
        switchOnOffd5.setOnClickListener {
            if (deviceModelVH.isOnline)
                ...
                deviceActionInterface.showSnackbar(switchOnOffd4.context.getString(R.string.device_offline))
                switchOnOffd5.isChecked = !switchOnOffd4.isChecked
            }
        }
